...
if module_exists('browser') and module_exists('javascript'):
   ...
   class GFX_Window(object):
      def __init__(self, width, height, onclose):
         ...
         js = 'function animateIt(dir, max, min) {var elem = document.getElementById("consoleArea"); var elem2 = document.getElementById("textarea"); var pos = max;var target = min;var id = setInterval(frame, 3);function frame() {var opa = Math.round((Math.abs(pos - target) / Math.abs(max - min)) * 100)/100;if (dir < 0) {opa = 1 - opa;} if (pos == target) {clearInterval(id);} else {pos = pos + dir; elem.style.width = pos + "px"; elem2.style.opacity = opa; }}}localStorage.switch = localStorage.switch ? localStorage.switch : "off";function switched() {if (localStorage.switch == "on") {localStorage.switch = "off";animateIt(-5, 300, 70);} else {localStorage.switch = "on";animateIt(5, 70, 300);}}'
